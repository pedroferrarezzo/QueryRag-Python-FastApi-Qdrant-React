...
from repository.vector_qdrant_repository import add_vector, add_vectors, search_vector

async def ingest_vector(vector_dto: VectorDto):
    ...
    await add_vector(vector)

async def ingest_vectors(vector_dtos: list[VectorDto]):
    ...
    await add_vectors(vectors)


async def search_documents(vector: list[float], k: int = 5) -> list[DocumentDto]:
    ...
    documents = await search_vector(vector, k)
